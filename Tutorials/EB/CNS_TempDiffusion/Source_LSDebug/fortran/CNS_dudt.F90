...
contains
  ...
  subroutine cns_eb_compute_dudt (lo,hi, dudt, utlo, uthi, &
       u,ulo,uhi,grad_eb,glo,ghi,grad_x,gxlo,gxhi,fx,fxlo,fxhi,fy,fylo,fyhi,fz,fzlo,fzhi,flag,fglo,fghi, &
    ...
    integer, dimension(3), intent(in) :: lo,hi,utlo,uthi,ulo,uhi,glo,ghi,gxlo,gxhi,&
    ...
    call eb_diff_mol_3d(lo, hi, dx, q, qlo, qhi, grad_x, gxlo, gxhi, lambda, mu, xi, clo, chi, &
  ...
  subroutine compute_primitive_variables (lo,hi,u,ulo,uhi,prim,plo,phi) &
    ...
    print*, "qlo is " , qlo(1), qlo(2), qlo(3)
    print*, "qhi is " , qhi(1), qhi(2), qhi(3)

    print*, "ulo is " , ulo(1), ulo(2), ulo(3)
    print*, "uhi is " , uhi(1), uhi(2), uhi(3)
    ...
    do k = qlo(3), qhi(3) 	
	  ...
	  do i = qlo(1), qhi(1)
		...
		if(i==25 .and. j==-4 .and. k==-4)then
			print*, u(i,j,k,1), u(i,j,k,2), u(i,j,k,3), u(i,j,k,4), u(i,j,k,5)
			print*, "Setting the value here ", q(i,j,k,qtemp)
		endif 
	  end do
      end do
   end do	 
