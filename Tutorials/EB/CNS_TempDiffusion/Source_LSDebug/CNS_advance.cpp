...
amrex::ParallelFor(amrex::grow(bx,5), 1,
                                [=] AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
                        {
				//if(i==25 && j==-4 && k==-4){
				//	std::cout << "Value is " << phi_arr(i,j,k,0);
				//	exit(0);
				//}
			});
	...
	print_grad_eb(BL_TO_FORTRAN_BOX(bx),
			...
			amrex::ParallelFor(amrex::grow(bx,3), 1,
				...
				if( iflag(i,j,k).isRegular() || iflag(i,j,k).isSingleValued()){
						...
						//std::cout<< i << " " << j << " " << k << " " <<  grad_x_arr(i,j,k,n) << "\n";
						//std::cout << "bcent " << bcent(i,j,k,0) << " " << bcent(i,j,k,1) << " " << bcent(i,j,k,2) << "\n";
						//std::cout << "ccent " << ccent(i,j,k,0) << " " << ccent(i,j,k,1) << " " << ccent(i,j,k,2) << "\n";
						//std::cout << "yloc, zloc"  << yloc_on_xface <<  " " <<  zloc_on_xface << "\n";
					...
					std::cout << "bx lovect is " <<  blo[0] << " " << blo[1] << " " << blo[2]  << "\n";
					std::cout << "bx hivect is " <<  bhi[0] << " " << bhi[1] << " " << bhi[2]  << "\n";
                                        std::cout << "Value is " << phi_arr(ii,jj,kk,0) << "\n";
							std::cout << ii << " " << jj << " " << kk << " " << phi_arr(ii,jj,kk,0) << " " << phi_eb_arr(ii,jj,kk,0) << "\n";
                                        //exit(0);
                                }
								}
							}
						}
						//std::cout<< grad_x_arr(i,j,k,n) << "\n";
						//if(iflag(i,j,k).isRegular()){
						//	std::cout <<"Regular" << "\n";
						//}
						//else{
						//	std::cout <<"Single" << "\n";
