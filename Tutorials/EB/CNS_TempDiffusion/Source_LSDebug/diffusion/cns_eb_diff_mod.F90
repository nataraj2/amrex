...
contains
  ...
  subroutine eb_diff_mol_3d (lo,hi, dx, &
    ...
    integer, intent(in) :: qd_lo(3), qd_hi(3), gxlo(3), gxhi(3)
    ...
    do       k = lo(3)-nextra-1, hi(3)+nextra+1
       do    j = lo(2)-nextra-1, hi(2)+nextra+1
          do i = lo(1)-nextra  , hi(1)+nextra+1
             !dTdx = grad_x(i,j,k,1) !(q(i,j,k,qtemp)-q(i-1,j,k,qtemp))*dxinv(1)
             dTdx = (q(i,j,k,qtemp)-q(i-1,j,k,qtemp))*dxinv(1)
	     !print*, i, j, k
	     !print*, "grad_x is ", grad_x(i,j,k,1)
	     !print*, "default is", (q(i,j,k,qtemp)-q(i-1,j,k,qtemp))*dxinv(1)	    
	     !print*, "---------------------------"
