...
use amrex_ebcellflag_module, only : is_regular_cell, is_covered_cell, is_single_valued_cell, &
  ...
  public:: cns_compute_dudt, cns_eb_compute_dudt, compute_primitive_variables
...
contains
  ...
  subroutine cns_compute_dudt (lo,hi, dudt, utlo, uthi, &
    ...
    fx(lo(1):hi(1)+1,lo(2):hi(2)  ,lo(3):hi(3)  ,1:4) = 0.d0
    fy(lo(1):hi(1)  ,lo(2):hi(2)+1,lo(3):hi(3)  ,1:4) = 0.d0
    fz(lo(1):hi(1)  ,lo(2):hi(2)  ,lo(3):hi(3)+1,1:4) = 0.d0


    call compute_divop (lo,hi,5,dx,dudt,utlo,uthi, &
         fx, fxlo, fxhi, fy, fylo, fyhi, fz, fzlo, fzhi)

    dudt(lo(1):hi(1),lo(2):hi(2),lo(3):hi(3),1:4) = 0.d0
  ...
  subroutine cns_eb_compute_dudt (lo,hi, dudt, utlo, uthi, &
       u,ulo,uhi,grad_eb,glo,ghi,grad_x,gxlo,gxhi,grad_y,gylo,gyhi,fx,fxlo,fxhi,fy,fylo,fyhi,fz,fzlo,fzhi,flag,fglo,fghi, &
    ...
    integer, dimension(3), intent(in) :: lo,hi,utlo,uthi,ulo,uhi,glo,ghi,gxlo,gxhi,gylo,gyhi,&
    ...
    real(rt), intent(in   ) :: grad_x ( gxlo(1): gxhi(1), gxlo(2): gxhi(2), gxlo(3): gxhi(3),1)
    real(rt), intent(in   ) :: grad_y ( gylo(1): gyhi(1), gylo(2): gyhi(2), gylo(3): gyhi(3),1)
    real(rt), intent(inout) :: fx(fxlo(1):fxhi(1),fxlo(2):fxhi(2),fxlo(3):fxhi(3),nvar)
    real(rt), intent(inout) :: fy(fylo(1):fyhi(1),fylo(2):fyhi(2),fylo(3):fyhi(3),nvar)
    real(rt), intent(inout) :: fz(fzlo(1):fzhi(1),fzlo(2):fzhi(2),fzlo(3):fzhi(3),nvar)
    ...
    call eb_diff_mol_3d(lo, hi, dx, q, qlo, qhi, grad_x, gxlo, gxhi, grad_y, gylo, gyhi, lambda, mu, xi, clo, chi, &
    ...
    fx(fxlo(1):fxhi(1),fxlo(2):fxhi(2),fxlo(3):fxhi(3),1:4) = 0.d0
    fy(fylo(1):fyhi(1),fylo(2):fyhi(2),fylo(3):fyhi(3),1:4) = 0.d0
    fz(fzlo(1):fzhi(1),fzlo(2):fzhi(2),fzlo(3):fzhi(3),1:4) = 0.d0
    ...
    call compute_eb_divop(lo,hi,5,dx,dt,fhx,lfxlo,lfxhi,fhy,lfylo,lfyhi,fhz,lfzlo,lfzhi,&
         fx, fxlo, fxhi, fy, fylo, fyhi, fz, fzlo, fzhi, &
         dudt,utlo,uthi, q,qlo,qhi, grad_eb, glo, ghi, lambda, mu, xi, clo, chi, &
  ...
  subroutine compute_primitive_variables (lo,hi,u,ulo,uhi,prim,plo,phi) &
       bind(c,name='compute_primitive_variables')
    use cns_nd_module, only : ctoprim
    integer, dimension(3), intent(in) :: lo,hi,ulo,uhi,plo,phi
    real(rt), intent(in   ) :: u ( ulo(1): uhi(1), ulo(2): uhi(2), ulo(3): uhi(3),nvar)
    real(rt), intent(inout) :: prim ( plo(1): phi(1), plo(2): phi(2), plo(3): phi(3),1)
    ...
    do k = qlo(3), qhi(3) 	
       do j = qlo(2), qhi(2)
	  do i = qlo(1), qhi(1)
		prim(i,j,k,1) = q(i,j,k,qtemp)
	  end do
      end do
   end do	 
	
    call amrex_deallocate(q)
  end subroutine compute_primitive_variables


  subroutine set_eb_dirichlet_for_prim(lo,hi,prim_eb,plo,phi) &
       bind(c,name='set_eb_dirichlet_for_prim')
    integer, dimension(3), intent(in) :: lo,hi,plo,phi
    real(rt), intent(inout) :: prim_eb ( plo(1): phi(1), plo(2): phi(2), plo(3): phi(3),1)
    ...
    do k = qlo(3), qhi(3) 	
       do j = qlo(2), qhi(2)
	  do i = qlo(1), qhi(1)
		prim_eb(i,j,k,1) = 500.0d0!287.9638541d0
	  end do
      end do
   end do	 
	
  end subroutine set_eb_dirichlet_for_prim

  subroutine set_grad_eb_val(lo,hi,grad_eb,glo,ghi) &
       bind(c,name='set_grad_eb_val')
    integer, dimension(3), intent(in) :: lo,hi,glo,ghi
    real(rt), intent(inout) :: grad_eb ( glo(1): ghi(1), glo(2): ghi(2), glo(3): ghi(3),1)
    ...
    do k = glo(3), ghi(3) 	
       do j = glo(2), ghi(2)
	  do i = glo(1), ghi(1)
		grad_eb(i,j,k,1) = 0.0d0
	  end do
      end do
   end do	 

 end subroutine set_grad_eb_val

  subroutine print_grad_eb(lo,hi,flag, flo, fhi, grad_eb,glo,ghi) &
       bind(c,name='print_grad_eb')
    integer, dimension(3), intent(in) :: lo, hi, flo, fhi, glo,ghi
    ...
    !print*, glo(1), ghi(1), glo(2), ghi(2), glo(3), ghi(3)
    ...
    do k = lo(3)-5, hi(3)+5
	  ...
	  do i = lo(1)-5, hi(1)+5
		...
		!if(i==-2 .and. j ==29 .and. k==-1)then
			if(i==25 .and. j==-4 .and. k==-4)then
			print*, "grad_eb_val", i,j,k,grad_eb(i,j,k,1)
			!stop
			endif
		!endif
		!endif
