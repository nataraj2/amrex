...
contains
  ...
  subroutine eb_diff_mol_3d (lo,hi, dx, &
       ...
       flux3, fd3_lo, fd3_hi, &
       flag, fglo, fghi)
    use amrex_ebcellflag_module, only : get_neighbor_cells, get_neighbor_cells_int_single, is_single_valued_cell
    integer, intent(in) :: qd_lo(3), qd_hi(3), gxlo(3), gxhi(3), gylo(3), gyhi(3)
    ...
    do       k = lo(3)-nextra-1, hi(3)+nextra+1
       do    j = lo(2)-nextra-1, hi(2)+nextra+1
	     ...
	     !print*, i, j, k, grad_x(i,j,k,1)*dxinv(1),(q(i,j,k,qtemp)-q(i-1,j,k,qtemp))*dxinv(1)

	     !if(is_single_valued_cell(flag(i,j,k)) .or. is_single_valued_cell(flag(i-1,j,k)) &
	     !.or. is_single_valued_cell(flag(i-2,j,k)) .or. is_single_valued_cell(flag(i+2,j,k)))then
             dTdx = grad_x(i,j,k,1)*dxinv(1)
	     !else	
    ...
    do       k = lo(3)-nextra-1, hi(3)+nextra+1
       do    j = lo(2)-nextra  , hi(2)+nextra+1
	     ...
	     !if(is_single_valued_cell(flag(i,j,k)) .or. is_single_valued_cell(flag(i,j-1,k)) .or.&
		!is_single_valued_cell(flag(i,j-2,k)) .or. is_single_valued_cell(flag(i,j+2,k)))then
             dTdy = grad_y(i,j,k,1)*dxinv(2)
	     !else
