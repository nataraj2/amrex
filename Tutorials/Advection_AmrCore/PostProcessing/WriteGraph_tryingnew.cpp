										...
										FILE* connect)
{
	...
	//std::cout << "Size of finemask is " << finemask.size() << "\n";
	//exit(0);
	...
	for (int lev=0; lev <= finest_lev; ++lev) {
		...
		const iMultiFab* finemask_mf_ptr;
		if(lev < finest_lev){
			finemask_mf_ptr = &finemask[lev];
		}
	
	    for (MFIter mfi(allmasks_mf); mfi.isValid(); ++mfi) {
	    	Array4<const int> const& allmasks_array = allmasks_mf.const_array(mfi);

			Array4<const int> const* finemask_array_ptr;
			if(lev < finest_lev) {
				auto&& temp_finemask_array = finemask_mf_ptr->const_array(mfi);
				finemask_array_ptr = &temp_finemask_array;
			}
			//Array4<const int> const& allmasks_array = *allmasks_array_ptr;

	        Box bx = mfi.validbox();
	        ParallelFor(bx, [=] AMREX_GPU_DEVICE (int i, int j, int k){
				if(k == 0 and allmasks_array(i,j,k,0) == 0 and &(*finemask_array_ptr)(i,j,k,0) == 0) {
	                ...
	                for(int i_shift=-3; i_shift<=3; i_shift++){
	                	for(int j_shift=-3; j_shift<=3; j_shift++){
	                		if(!(i_shift == 0  and j_shift == 0)){
	                        	FindNeighbors(lev, i+i_shift, j+j_shift, k, allmasks_array, *finemask_array_ptr, dx0, plo, vec_coords, quad);
	                        }
	                    }
	                }
	                ...
	                if(&(*finemask_array_ptr)(i,j+1,k,0) == 0 and allmasks_array(i,j+1,k,0) == 2){
